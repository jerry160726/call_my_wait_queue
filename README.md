...
System call 的具體功能由 Kernel 中的 `SYSCALL_DEFINE` marco 實現
```c=44
...
LIST_HEAD(my_list);                     // 定義一個list head，儲存 process 資料
static DEFINE_MUTEX(my_mutex);          // 定義互斥鎖，保護共享資源
static int condition = 0;               // 等待佇列的條件
```
* `my_wait_queue` : 使用 Kernel 提供的 marco `DECLARE_WAIT_QUEUE_HEAD`，宣告一個名為 `my_wait_queue` 的 wait queue。
...
* 如果 `id == 1`：獲取互斥鎖 `my_mutex`，防止 Multi processing 同時操作 `my_list`。
    * 呼叫 `enter_wait_queue()`，將當前 process 加入 wait queue。
    * 最後釋放互斥鎖。
* 如果 `id == 2`：呼叫 `clean_wait_queue()`，依次喚醒 wait queue 中的所有 process。